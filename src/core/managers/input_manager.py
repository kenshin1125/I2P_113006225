...
class InputManager:
    ...
    def handle_events(self, e: pg.event.Event) -> None:
        if e.type == pg.MOUSEMOTION:
            ...
            # collect printable characters for chat
            if hasattr(e, "unicode") and e.unicode and len(e.unicode) == 1 and e.unicode.isprintable():
                self._text_buffer.append(e.unicode)
    ...
    def consume_text_input(self) -> list[str]:
        """Return typed characters for this frame and clear buffer."""
        chars = list(self._text_buffer)
        self._text_buffer.clear()
        return chars
