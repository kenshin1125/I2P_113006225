...
class Player:
    ...
    def update(self, x: float, y: float, map: str, direction: str | None = None, moving: bool | None = None) -> None:
        if x != self.x or y != self.y or map != self.map or (direction and direction != self.direction) or (moving is not None and moving != self.moving):
